...
const Services = () => {
  ...
  return (
    <section id="services" className="py-16 md:py-20 bg-brand-light-gray/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-6 md:gap-8">
          {services.map((service, index) => (
            <div 
              key={index}
              className="group bg-background rounded-2xl p-6 md:p-8 shadow-elegant hover:shadow-glow transition-all duration-300 hover:-translate-y-2"
            >
              <div className="w-16 h-16 bg-brand-deep-blue rounded-xl flex items-center justify-center mb-6 group-hover:bg-brand-mid-blue transition-colors duration-300">
                <service.icon className="w-8 h-8 text-brand-white" />
              </div>
              
              <h3 className="font-brand-heading text-xl mb-4 text-brand-dark-navy group-hover:text-brand-deep-blue transition-colors duration-300">
                {service.title}
              </h3>
              
              <p className="font-brand-body text-brand-dark-navy/70 leading-relaxed">
                {service.description}
              </p>
            </div>
          ))}
